...
import com.example.news_summarizer.dto.*; // 카카오 사용자 정보 DTO
...
public class LoginService {
    ...
    @Value("${google.redirect-uri}") // 백엔드에서 Google로 요청할 때 사용하는 리다이렉션 URI
    ...
    public User authenticateKakaoUser(LoginRequest loginRequest) {
        ...
        if (existingUser.isPresent()) {
            ...
            System.out.println("기존 카카오 사용자 로그인: " + user.getNickname());
        } else {
            user = User.builder()
                    .socialId(id) // 카카오 고유 ID를 socialId에 저장
    ...
    public Mono<JsonNode> getGoogleAccessToken(String code) {
        ...
        return webClient.post()
                ...
                .retrieve()
                .bodyToMono(JsonNode.class)
                .doOnError(e -> System.err.println("Google 토큰 획득 요청 실패: " + e.getMessage()));
    }
